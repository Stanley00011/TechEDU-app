...
if(isset($_POST['update'])){
   ...
   if(!empty($thumb)){
      if($thumb_size > 2000000){
         ...
         if($old_thumb != '' AND $old_thumb != $rename_thumb){
            unlink('./../../images/upload_files/'.$old_thumb);
         }
      }
   }
   ...
   $video_folder = './../../images/upload_files/'.$rename_video;
   ...
   if(!empty($video)){
      ...
      if($old_video != '' AND $old_video != $rename_video){
         unlink('./../../images/upload_files/'.$old_video);
      }
   }
...
if(isset($_POST['delete_video'])){
   ...
   unlink('./../../images/upload_files/'.$fetch_thumb['thumb']);

   $delete_video = $conn->prepare("SELECT video FROM `content` WHERE id = ? LIMIT 1");
   $delete_video->execute([$delete_id]);
   $fetch_video = $delete_video->fetch(PDO::FETCH_ASSOC);
   unlink('./../../images/upload_files/'.$fetch_video['video']);
...
<section class="video-form">
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <img src="./../../images/upload_files/<?= $fecth_videos['thumb']; ?>" alt="">
      <p>update thumbnail</p>
      <input type="file" name="thumb" accept="image/*" class="box">
      <video src="./../../images/upload_files/<?= $fecth_videos['video']; ?>" controls></video>
      <p>update video</p>
